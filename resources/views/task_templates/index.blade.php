...
<div class="row g-3">
  ...
  <div class="col-md-3"><label class="form-label">Каждые N</label><input name="recurrence_interval" type="number" min="1" value="1" class="form-control"><div class="form-text" id="intervalHint">Например 14 = каждые 14 дней</div></div>
  ...
  <div class="col-md-4 recurrence-extra d-none" id="monthdayWrap"><label class="form-label">Число месяца</label><input name="day_of_month" type="number" min="1" max="31" value="1" class="form-control"><div class="form-text">Например 5 = пятого числа</div></div>
...
function loadTemplates(){$.get('{{ route('task-templates.index') }}',r=>{templateCache={};let h='';r.forEach(t=>{templateCache[t.id]=t;const checks=(t.checklist_items||[]).map(i=>`<li>${escT(i.title)}</li>`).join('');h+=`<div class="col-12 col-xl-6"><div class="card border-0 shadow-sm h-100"><div class="card-body"><div class="d-flex gap-2"><div class="flex-grow-1"><h5>${escT(t.title)}</h5><div class="text-muted small">${escT(t.assignee?t.assignee.last_name+' '+t.assignee.first_name:'Исполнитель = автор')} · ${escT(recName(t))} · срок +${t.due_after_days} дн.</div></div><span class="badge ${t.is_active?'text-bg-success':'text-bg-secondary'}">${t.is_active?'Активен':'Отключён'}</span></div><p class="mt-3 mb-2">${escT(t.description||'')}</p>${checks?`<ol class="small mb-3">${checks}</ol>`:''}<div class="d-flex align-items-center gap-2 flex-wrap"><span class="badge text-bg-light border">${escT(t.priority)}</span>${t.next_run_at?`<span class="small text-muted">Следующий запуск: ${new Date(t.next_run_at).toLocaleString('ru-RU')}</span>`:''}<button class="btn btn-sm btn-outline-primary ms-auto" onclick="editTemplate(${t.id})"><i class="bi bi-pencil"></i></button><button class="btn btn-sm ${t.is_active?'btn-outline-secondary':'btn-outline-success'}" onclick="toggleTemplate(${t.id},${t.is_active?0:1})">${t.is_active?'Отключить':'Включить'}</button><button class="btn btn-sm btn-primary" onclick="runTemplate(${t.id})"><i class="bi bi-play-fill me-1"></i>Создать сейчас</button></div></div></div></div>`});$('#templateList').html(h||'<div class="col-12"><div class="alert alert-light border text-center">Шаблонов пока нет</div></div>')})}
function newTemplate(){$('#templateForm')[0].reset();$('#templateId').val('');$('#templateActive').prop('checked',true);$('#checklistEditor').empty();addChecklistEditor();toggleRecurrence();$('#templateError').addClass('d-none')}
function editTemplate(id){let t=templateCache[id];newTemplate();$('#templateId').val(id);for(let k of ['title','assigned_to','description','priority','due_after_days','recurrence','recurrence_interval','weekday','day_of_month'])$(`[name=${k}]`).val(t[k]??'');if(t.next_run_at)$('[name=next_run_at]').val(t.next_run_at.slice(0,16));$('#templateActive').prop('checked',!!t.is_active);$('#checklistEditor').empty();(t.checklist_items||[]).forEach(x=>addChecklistEditor(x.title));if(!(t.checklist_items||[]).length)addChecklistEditor();toggleRecurrence();bootstrap.Modal.getOrCreateInstance(document.getElementById('templateModal')).show()}
...
$('#templateForm').on('submit',function(e){e.preventDefault();let id=$('#templateId').val(),a=$(this).serializeArray(),d={};a.forEach(x=>{if(x.name==='checklist[]'){d.checklist=d.checklist||[];d.checklist.push(x.value)}else d[x.name]=x.value});d.is_active=$('#templateActive').is(':checked')?1:0;$.ajax({url:id?`{{ url('/ajax/task-templates') }}/${id}`:'{{ route('task-templates.store') }}',method:id?'PATCH':'POST',data:d}).done(()=>{bootstrap.Modal.getInstance(document.getElementById('templateModal')).hide();loadTemplates()}).fail(x=>$('#templateError').removeClass('d-none').text(x.responseJSON?.message||'Ошибка сохранения'))});
