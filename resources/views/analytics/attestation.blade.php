...
@else
@php
$relationChart=['labels'=>['Оценка 1','Оценка 2','Оценка 3','Оценка 4'],'values'=>array_values($relationDistribution)];
$commissionChart=['labels'=>['Оценка 1','Оценка 2','Оценка 3','Оценка 4'],'values'=>array_values($commissionDistribution)];
$departmentCommissionValues=[]; foreach($departmentRows as $row){$departmentCommissionValues[]=(float)($row['commission_avg']??0);}
$departmentChart=['labels'=>$departmentRows->pluck('name')->values()->all(),'values'=>$departmentCommissionValues];
$memberLabels=[]; foreach($memberProgress as $row){$memberLabels[]=$row['user']->full_name;}
$memberChart=['labels'=>$memberLabels,'values'=>$memberProgress->pluck('percent')->values()->all()];
@endphp
...
<div class="row g-3 mb-3">
<div class="col-lg-6"><div class="card border-0 shadow-sm h-100"><div class="card-header bg-white"><b>Распределение рабочих связей</b></div><div class="card-body"><canvas data-height="300" data-crm-chart="donut" data-chart='@json($relationChart)'></canvas></div></div></div>
<div class="col-lg-6"><div class="card border-0 shadow-sm h-100"><div class="card-header bg-white"><b>Распределение оценок комиссии</b></div><div class="card-body"><canvas data-height="300" data-crm-chart="donut" data-chart='@json($commissionChart)'></canvas></div></div></div>
</div>
<div class="card border-0 shadow-sm mb-3"><div class="card-header bg-white"><b>Средние оценки по подразделениям</b></div><div class="card-body"><canvas data-height="320" data-crm-chart="bar" data-chart='@json($departmentChart)'></canvas></div></div>
