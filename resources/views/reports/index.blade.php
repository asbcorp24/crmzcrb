...
<div class="card border-0 shadow-sm mb-3"><div class="card-body"><div class="row g-2 align-items-end">
  ...
  <div class="col-md-8 d-flex gap-2 justify-content-md-end flex-wrap"><button class="btn btn-primary" onclick="loadReport()"><i class="bi bi-funnel me-1"></i>Сформировать</button><button class="btn btn-outline-success" onclick="exportExcel()"><i class="bi bi-file-earmark-spreadsheet me-1"></i>Excel</button><button class="btn btn-outline-secondary" onclick="exportCsv()"><i class="bi bi-filetype-csv me-1"></i>CSV</button><button class="btn btn-outline-danger" onclick="exportPdf()"><i class="bi bi-file-earmark-pdf me-1"></i>PDF</button></div>
</div></div></div>
...
function exportExcel(){window.location.href='{{ route('reports.excel') }}?'+queryString()}
