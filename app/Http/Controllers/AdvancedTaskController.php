...
class AdvancedTaskController extends Controller
{
    ...
    public function bulkStore(Request $request)
    {
        ...
        $data=$request->validate([
            ...
            'department_id'=>'nullable|exists:departments,id','position_id'=>'nullable|exists:positions,id',
        ...
        $targets=match($data['target_type']){
            ...
            'department'=>User::where('department_id',$data['department_id']??0)->where('is_active',true)->pluck('id'),
            'managers'=>User::whereIn('id',$allowed)->where('role','manager')->where('is_active',true)->pluck('id'),
            'position'=>EmployeeAssignment::whereNull('ended_at')->whereHas('staffingPosition',fn($q)=>$q->where('position_id',$data['position_id']??0))->pluck('user_id'),
        };
        $targets=$targets->unique()->filter(fn($id)=>$allowed->contains((int)$id))->values();
        abort_if($targets->isEmpty(),422,'Не найдено доступных исполнителей');
    ...
    public function metadata(Request $request, Task $task)
    {
        ...
        return response()->json(['subtasks'=>$task->subtasks()->with('assignee')->get(),'blockers'=>$task->blockers()->with('assignee')->get(),'blocked_tasks'=>$task->blockedTasks()->with('assignee')->get(),'tags'=>$task->tags()->get(),'available_tags'=>TaskTag::where('is_active',true)->orderBy('name')->get()]);
    }
    ...
    public function addDependency(Request $request, Task $task)
    {
        ...
        $task->blockers()->syncWithoutDetaching([$blocker->id=>['created_by'=>$request->user()->id]]);
    ...
    {
        abort_unless($request->user()->isManager(),403); $data=$request->validate(['name'=>'required|string|max:100']);
        $tag=TaskTag::firstOrCreate(['slug'=>Str::slug($data['name'])],['name'=>$data['name'],'is_active'=>true]); return response()->json(['ok'=>true,'tag'=>$tag],201);
    }
