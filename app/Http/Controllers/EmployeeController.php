...
class EmployeeController extends Controller
{
    public function page(Request $request)
    {
        ...
        return view('employees.index', [
            ...
            'managers' => User::whereIn('id', $ids)->whereIn('role', ['admin','manager'])->where('is_active', true)->whereNull('archived_at')->orderBy('last_name')->get(),
        ]);
    }

    public function profile(Request $request, User $employee)
    {
        $access = app(AccessService::class); abort_unless($access->canViewUser($request->user(), $employee), 403);
        $employee->load(['department','manager','subordinates']);
        $taskBase = Task::whereNull('archived_at')->where('assigned_to', $employee->id);
        $stats = [
            'all' => (clone $taskBase)->count(),'open' => (clone $taskBase)->whereNotIn('status', ['completed','cancelled'])->count(),
            'overdue' => (clone $taskBase)->whereNotIn('status', ['completed','cancelled'])->whereNotNull('due_at')->where('due_at','<',now())->count(),
            'review' => (clone $taskBase)->where('status','review')->count(),'completed' => (clone $taskBase)->where('status','completed')->count(),
    ...
    public function index(Request $request)
    {
        ...
        $q = User::with(['department','manager'])->whereIn('id', $ids)->whereNull('archived_at');
    ...
    public function store(Request $request)
    {
        ...
        $data['password'] = Hash::make($request->input('password', 'ChangeMe123!')); $user = User::create($data);
        return response()->json(['ok'=>true,'user'=>$user->load(['department','manager'])], 201);
    }

    public function update(Request $request, User $employee)
    {
        $access = app(AccessService::class); abort_unless($request->user()->isManager() && ($request->user()->isAdmin() || $access->canManageUser($request->user(), $employee)), 403);
        $data = $this->validated($request, $employee->id);
        if (!$request->user()->isAdmin()) { abort_if(($data['role'] ?? $employee->role) === 'admin', 403); if (array_key_exists('manager_id', $data) && $data['manager_id']) { $allowedManagers = $access->userIds($request->user(), true); abort_unless($allowedManagers->contains((int)$data['manager_id']), 403, 'Нельзя назначить руководителя вне доступной иерархии'); } }
        if ($request->filled('password')) $data['password'] = Hash::make($request->password); $employee->update($data);
        return response()->json(['ok'=>true,'user'=>$employee->fresh()->load(['department','manager'])]);
    }

    private function validated(Request $request, ?int $id = null): array
    {
        return $request->validate(['department_id'=>'nullable|exists:departments,id','manager_id'=>'nullable|exists:users,id','last_name'=>'required|string|max:100','first_name'=>'required|string|max:100','middle_name'=>'nullable|string|max:100','position'=>'required|string|max:190','email'=>['required','email','max:190',Rule::unique('users','email')->ignore($id)],'phone'=>'nullable|string|max:50','role'=>['required',Rule::in(['admin','manager','employee'])],'is_active'=>'required|boolean','employment_date'=>'nullable|date','password'=>'nullable|string|min:8|max:100']);
    }
}
