...
class MeetingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $q = Meeting::with(['chairman','secretary','creator','items.assignee','items.task.assignee.department'])
            ->where(function ($w) use ($request, $ids) {
                $w->whereIn('created_by', $ids)
                  ->orWhereHas('participants', fn($p) => $p->where('users.id', $request->user()->id));
            });
    ...
    public function store(Request $request)
    {
        ...
        foreach ($participants as $id) abort_unless($allowed->contains((int)$id), 403);
        unset($data['participants']);
        $data['created_by']=$request->user()->id;

        $meeting=DB::transaction(function() use($data,$participants){
            $m=Meeting::create($data);
            $m->participants()->sync($participants);
            return $m;
        });
    ...
    public function addItem(Request $request, Meeting $meeting)
    {
        ...
        $item=DB::transaction(function() use($request,$meeting,$data){
            $number=(int)$meeting->items()->lockForUpdate()->max('number')+1;
            $task=Task::create([
    ...
    {
        $user = $request->user();
        if ($user->isAdmin() || (int)$meeting->created_by === (int)$user->id) return;
        $ids = app(AccessService::class)->userIds($user, true);
        if ($ids->contains((int)$meeting->created_by)) return;
        abort_unless($meeting->participants()->where('users.id', $user->id)->exists(), 403);
    }
    ...
    private function authorizeMeetingManage(Request $request, Meeting $meeting): void
    {
        ...
        abort_unless($creator && app(AccessService::class)->canManageUser($user, $creator), 403);
    }
}
