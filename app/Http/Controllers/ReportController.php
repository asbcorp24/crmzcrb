...
class ReportController extends Controller
{
    ...
    public function data(Request $request)
    {
        ...
        return response()->json($this->buildReport($request, $type, $from, $to));
    }
    ...
    public function excel(Request $request)
    {
        ...
        $name = 'crm_report_'.$type.'_'.$from->format('Y-m-d').'_'.$to->format('Y-m-d').'.xls';
        $html = '<!doctype html><html><head><meta charset="UTF-8"></head><body><table border="1"><thead><tr>';
        foreach ($report['headers'] as $h) $html .= '<th>'.e($h).'</th>';
        $html .= '</tr></thead><tbody>';
        foreach ($report['rows'] as $row) {
            $html .= '<tr>';
            foreach ($report['keys'] as $key) $html .= '<td>'.e((string)($row[$key] ?? '')).'</td>';
            $html .= '</tr>';
        }
    ...
    public function csv(Request $request): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($report) {
            $out = fopen('php://output', 'w'); fwrite($out, "\xEF\xBB\xBF"); fputcsv($out, $report['headers'], ';');
            foreach ($report['rows'] as $row) { $line=[]; foreach ($report['keys'] as $key) $line[]=$row[$key]??''; fputcsv($out,$line,';'); }
            fclose($out);
    ...
    private function filters(Request $request): array
    {
        ...
        $from=$request->date('date_from')?->startOfDay()??now()->startOfMonth(); $to=$request->date('date_to')?->endOfDay()??now()->endOfMonth();
        abort_if($to->lt($from),422,'Конечная дата не может быть раньше начальной'); return [$type,$from,$to];
    }

    private function buildReport(Request $request,string $type,$from,$to): array
    { return match($type){'employees'=>$this->employeesReport($request,$from,$to),'departments'=>$this->departmentsReport($request,$from,$to),'meetings'=>$this->meetingsReport($request,$from,$to),default=>$this->tasksReport($request,$from,$to)}; }

    private function taskQuery(Request $request,$from,$to): Builder
    {
        $ids=$this->accessibleUsers($request)->pluck('id');
        $q=Task::with(['assignee.department','creator','overdueReasons'])->whereIn('assigned_to',$ids)->where(function($w)use($from,$to){$w->whereBetween('due_at',[$from,$to])->orWhereBetween('completed_at',[$from,$to])->orWhereBetween('created_at',[$from,$to]);});
        ...
        if($request->filled('status'))$q->where('status',$request->status); return $q;
    }
    ...
    {
        $users=$this->accessibleUsers($request)->with('department')->when($request->filled('user_id'),fn($q)=>$q->where('id',$request->integer('user_id')))->when($request->filled('department_id'),fn($q)=>$q->where('department_id',$request->integer('department_id')))->get();
        $tasks=$this->taskQuery($request,$from,$to)->get()->groupBy('assigned_to');
        $rows=$users->map(function($u)use($tasks){$set=$tasks->get($u->id,collect());$total=$set->count();$done=$set->where('status','completed')->count();return ['employee'=>$u->full_name,'department'=>$u->department?->name,'position'=>$u->position,'total'=>$total,'completed'=>$done,'review'=>$set->where('status','review')->count(),'overdue'=>$set->filter(fn($t)=>$t->is_overdue)->count(),'percent'=>$total?round($done/$total*100).'%':'—'];});
    ...
    {
        $tasks=$this->taskQuery($request,$from,$to)->get();$groups=$tasks->groupBy(fn($t)=>$t->assignee?->department?->name?:'Без подразделения');
        $rows=$groups->map(function($set,$name){$total=$set->count();$done=$set->where('status','completed')->count();return ['department'=>$name,'employees'=>$set->pluck('assigned_to')->unique()->count(),'total'=>$total,'completed'=>$done,'review'=>$set->where('status','review')->count(),'overdue'=>$set->filter(fn($t)=>$t->is_overdue)->count(),'percent'=>$total?round($done/$total*100).'%':'—'];})->values();
    ...
    {
        $userIds=$this->accessibleUsers($request)->pluck('id');$q=Meeting::with(['items.task.assignee.department'])->whereBetween('held_at',[$from,$to]);if(!$request->user()->isAdmin())$q->where('created_by',$request->user()->id);$meetings=$q->orderBy('held_at')->get();$rows=collect();
        foreach($meetings as $m)foreach($m->items as $i){$t=$i->task;if(!$t||!$userIds->contains($t->assigned_to))continue;if($request->filled('user_id')&&$t->assigned_to!=$request->integer('user_id'))continue;if($request->filled('department_id')&&$t->assignee?->department_id!=$request->integer('department_id'))continue;if($request->filled('status')&&$t->status!==$request->status)continue;$rows->push(['meeting'=>$m->title,'held_at'=>$m->held_at->format('d.m.Y H:i'),'number'=>$i->number,'instruction'=>$i->instruction,'employee'=>$t->assignee?->full_name,'department'=>$t->assignee?->department?->name,'due_at'=>$t->due_at?->format('d.m.Y H:i'),'status'=>$this->statusName($t->status),'progress'=>$t->progress.'%','overdue'=>$t->is_overdue?'Да':'Нет']);}
